...
Some other metrics are implemented in `yardstick`. You can check out other metric types in this [article](https://yardstick.tidymodels.org/articles/metric-types.html). For instance there is one called `huber_loss()` which is a combination of least squares and absolute error. 
...
The [model evaluation section of HMLR](https://bradleyboehmke.github.io/HOML/process.html#model-eval) lists and describes various performance metrics. The [Performance chapter of _Tidy Modeling with R_](https://www.tmwr.org/performance.html) also discusses different performance metrics.
