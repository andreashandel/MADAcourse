...
Another good overview and introduction, at a slightly more abstract level, is given in the [_linear regression_ chapter in ISLR](https://statlearning.com/). You should read through this chapter in at least enough detail to get the big picture ideas. Depending on your interest and willingness to look at equations, try to follow along with the math as well. But the important part is to understand the overall ideas.
...
But wait, there is more! Yet another discussion of linear models is in the [_linear regression_ chapter in HMLR](https://bradleyboehmke.github.io/HOML/linear-regression.html).
...
At the end of the [_classification_ chapter in ISLR](https://statlearning.com/), there is a nice brief discussion of GLMs, I suggest you read through that in enough depth to get the overall ideas, but there is no need to go through all the math.
...
If you never thought about the relation between classical statistical tests and GLM-type models, you might want to check out [_Common statistical tests are linear models_](https://lindeloev.github.io/tests-as-linear/) by [Jonas Kristoffer Lindeløv](https://lindeloev.net/) which explains the relation between the two in a good bit of detail. [_Statistical tests as linear models_](https://steverxd.github.io/Stat_tests/) by [Steve Doogue](https://stevedoogue.netlify.app/) is also good, it is based on the original text by Lindeløv and covers some topics in more detail. Both resources show examples in R, and have links to further materials. At some point in your career as a data analyst, you might need to use or interpret classical statistical tests, and having a general idea that they can map to GLM-type and other models might be helpful.
