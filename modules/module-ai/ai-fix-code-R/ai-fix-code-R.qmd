...
The overall goal is to get working code as quickly as possible. The goal is not to have it all AI generated. Therefore, if you reach a stage where you realize it's faster if you just fix the code yourself, go ahead and do so. 

That occurs at two stages. The most obvious one is close to the end. The AI has given you code that is very close to what you want, but there are a few issues, either conceptual or coding errors, that are not quite the way they should be, but when you see the code, you immediately know how to make those changes. Then just go ahead and finish things off manually. At that point, you should be able to "turn off" the AI and have working code that you can apply to your project.

The other time you might want to intervene manually is if the AI gets stuck. You keep asking it to fix/update, and it just doesn't produce what you want. At this stage, you could go ahead and make some manual adjustments to the code that you know should happen, and then feed the -- maybe still not fully working -- code back to the AI and ask it to update further. 
