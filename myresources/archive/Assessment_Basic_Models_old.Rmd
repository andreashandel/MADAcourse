...
For this exercise, we'll revisit and extend the project you started in the _wrangling_ module. Make sure you have the latest version of the repository on your local machine (it might not be, since last time your classmate contributed to your repository). As needed pull/push/merge to make sure remote and local repositories are in sync. Open the project. 
...
We already spent time on the important steps of data wrangling/cleaning and exploring/visualizing. But practice makes perfect 😀 so we'll talk a bit more and do a bit more if it here.
