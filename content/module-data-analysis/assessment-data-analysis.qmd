...
Take a look inside the `starter-analysis-exercise` folder. You will find files and folders that provide a template for a data analysis. This is a simplified version of a Github template called [dataanalyis-template](https://github.com/andreashandel/dataanalysis-template). You'll be using the full template for your class project later in the course. For now, go to the github repository for the [dataanalysis-template](https://github.com/andreashandel/dataanalysis-template) and read the information in the `README.md` file (shown at the bottom of the repository website). You can ignore the bits about `renv`, we are not using that just yet.
...
Now we are ready to include the new findings. Open `starter-analysis-report.qmd` inside the `products/report` folder. Add code to display the two new figures and the new table. Add a few sentences to each to explain what it shows and what it means.
