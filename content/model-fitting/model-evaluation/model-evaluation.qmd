...
This unit discusses the idea of assessing a model based on its performance. This is a general overview, additional units will go into more details of specific ways to assess performance.

::: note
I use the term performance here in a narrow sense, you will realize how I define it as you go through this unit.
:::
...
What is important that for each scenario, model performance is clearly defined. What you mean by _model being close to data_ or _model capturing the pattern in the data_ needs to be to specified such that your algorithm can compute a number that it can try to optimize - the performance measure.

It is important to be aware that the choice of the performance measure is a scientific choice. You need to specify it based on your knowledge of the system and the question you want to answer. While there are very common performance measures, which we will discuss next, it is important that you do think about what it is you are actually asking your model to do and what you optimize when you fit data.



# The cost/loss/objective function

To specify a performance measure we want to optimize, we need to define some function of both model predictions (*Y~m~*) and measured/data (*Y~d~*) outcomes. This function goes by different names, but the terms most commonly used are **cost function** or [**loss function**](https://en.wikipedia.org/wiki/Loss_function). In more general math or stats context, this is also called an **objective function.** Other names are common in different areas of science, see e.g., those listed in the linked Wikipedia article. 
...
When you do a single model fitting step, the computer calculates *f* for different values of the parameters, *b~i~*. In general, it repeats this for different values of the model parameters until it finds the ones that produce the best (i.e., minimum) value of *C*.[^2]

[^2]: Technically, for some simple models, it is possible to find the best parameter values other than by 'trial and error', but for many other models, this is more or less what is happening. 
...
Robust estimation methods such as MAE and many others often require the use of specialized software or packages that provide this functionality. 

R packages. A large number of R packages exist that allow fitting this way, see e.g., the [CRAN Robust Task View](https://cran.r-project.org/web/views/Robust.html). The `yardstick` package as the metric `mae()` which computes the mean absolute error.
