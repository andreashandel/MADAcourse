...
Audio data is a subfield of **signal processing**, which is a field of math and statistics that also can be applied to analyze
data from optical heart rate monitors and chemical spectroscopy data, among other things.
...
hierarchical data we often think about nested in terms of statistical clusters.
...
internet the past few years, and is essentially the *lingua franca* of APIs.
While statisticians often store hierarchical data in one or more rectangular
data sets, recursively nested data is stored in recursive lists, which are not always symmetrical. For example, consider the following
...
where each study unit is only measured a handful of times which may be spread apart. Most cohort studies therefore fall into this *longitudinal* framework, not into a *time series* framework. The hierarchical (or mixed effects) methods discussed earlier are typically more appropriate for this type of data.
