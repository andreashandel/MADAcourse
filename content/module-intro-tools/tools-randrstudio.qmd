...
-   If you haven't already, [install R first](https://www.r-project.org/). You can pick any mirror you like. If you already have R installed, make sure it is a fairly recent version, at least version 4.2 or newer. I recommend you upgrade to the most recent version (as of this writing 4.4.2, but if there is a newer one available, use that one).
...
![Installing an R package. Click on the image to enlarge it.](../../media/installpackagesscreenshot.png){fig-alt="Screenshot of RStudio with the command 'install.packages(learnr)' in the console."}
